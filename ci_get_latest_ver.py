...
def main(ET, SERVICE, CONF_DIR):
    ...
    def createCommonVars(GetCOMMON):

        ### Set vars from conf file
        ARTIFACTORY = GetCOMMON['URL']
        SERVICE_LIST = GetCOMMON['SERVICE_LIST']
        return [ARTIFACTORY, SERVICE_LIST]

    common_vars = createCommonVars(conf_vars[0])  # conf_vars[0] -- GetCOMMON from createConfVars

    ### R
    SERVICE_LIST = [item.encode('ascii').strip() for item in common_vars[1].split(',')]




    def createServiceVars(GetService):
        ...
        return [SERVICE_REPO, SERVICE_GROUP_ID, SERVICE_ARTIFACT_ID]

    service_vars = createServiceVars( conf_vars[1])  # conf_vars[1] -- GetService from createConfVars
    ...
    addr = GetXML(common_vars[0], service_vars[0], service_vars[1], service_vars[2]) # common_vars[0] --- ARTIFACTORY from createCommonVars, service_vars[0] -- SERVICE_REPO from createServiceVars
    ...
    def write_to_file(SERVICE, latest_artifact):
        file_name = "vars_env.prorerties"
        f = open('{}'.format(file_name), 'a')
        f.write('{}={}\n'.format(SERVICE, latest_artifact))
        f.close()

    write_to_file(SERVICE, latest_artifact)
...
if __name__ == '__main__':
    ...
    def createParser():
        ...
        parser.add_argument('-s', '--service', default='WAS', help='Set service type')
        return parser
    ...
    main(ET, SERVICE, CONF_DIR)


#    list = ['WAS', 'CDS_BE', 'CMS_FE']
    ### Run the main function
#    for item in list:
#        main('ET2', '{}'.format(item))
